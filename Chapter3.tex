...
\end{proof}
\begin{remark}
	If we replace the constant $\frac{1}{2}$ in the definition of transitions lifting with an arbitrary discount factor $\lambda \in \interval[open]{0}{1}$ (as discussed in \Cref{c3:rem:discount}), the Lipschitz constant in the preceding lemma would become $\lambda$ while preserving the contractivity of $\Phi_\beta$.
\end{remark}
...
\end{proof}
\begin{remark}
If the discount factor $\frac{1}{2}$ in the definition of transition lifting were replaced with an arbitrary $\lambda \in \interval[open]{0}{1}$ (as discussed in \Cref{c3:rem:discount}), the lemma above would require modification: distances would instead be either zero or of the form $\lambda^k$, for some $k \in \N$.
\end{remark}
