...
Kozen’s approach characterises the Kleene star as the least fixpoint with respect to a natural order induced by the idempotent $+$ operation~\cite{Kozen:1994:Completeness}, which corresponds to simulations of deterministic automata. However, for PRE, defining an analogous order that captures simulations of GPTS and supports a least-fixpoint interpretation of the probabilistic Kleene star remains a challenge. Although there are equational variants of Kozen’s axiomatisation that introduce an inference rule to capture the uniformity (in the sense of Bloom and Ésik~\cite{Bloom:1993:Iteration}) of Kleene star~\cite{Goncharov:2024:Unifying}, it is still unclear how to adapt Kozen’s completeness proof to the probabilistic setting. In particular, his argument depends on lifting operations on regular expressions to matrices of regular expressions and relating them to automata. For PRE, defining a probabilistic Kleene star on matrices that behaves well enough to support such a lifting remains a significant challenge. For these reasons, we adopt Salomaa’s older, automata-theoretic approach. Notably, this style of axiomatisation often appears in the works from the field of process algebra~\cite{Milner:1984:Complete}.
