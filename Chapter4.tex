...
for regular expressions and nondeterminisic automata~\cite{Antimirov:1996:Partial}. Given $a \in \alphabet$, $e,f \in \PExp$ and $p \in \interval{0}{1}$ we define:
\begin{gather*}
    \partial(\zero) = \emptydist \quad\quad \partial(\one)=\delta_{\checkmark} \quad\quad \partial(a)=\delta_{(a,\one)}  \quad\quad  \partial(e \oplus_p f)=p\partial(e) + (1-p)\partial(f)
\end{gather*}
