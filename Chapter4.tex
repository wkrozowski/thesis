...
\begin{definition}\label{c4:def:rerouting}
	Let $f \in \PExp$. We define $( - \lhd f) : \distf \funF \PExp \to \distf \funF \PExp$ to be given by $( - \lhd f) = {c_f}^{\sharp}$, i.e. a convex extension of the map $c_f : 1+A\times \PExp \to \distf (1+A \times \PExp)$ given by the following:
	\begin{gather*}
		c_f(x) = \begin{cases}
...
In the definition of $\partial(e;f)$, we use the helper function $( - \lhd f)$ defined above (see \Cref{c4:def:rerouting}). Intuitively speaking, we need to factor in the possibility that $e$ may accept with some probability $t$, in which case the outgoing transitions of $f$ contribute to the outgoing transitions of $e \seq f$. In such a case, $( - \lhd f)$ reroutes the transitions coming out of ${\partial(e)}$: acceptance (the first case of $c_f$ from \Cref{c4:def:rerouting}) is replaced by the behaviour of ${f}$, and the probability mass of transitioning to ${e}'$ (the second case of $c_f$ from \Cref{c4:def:rerouting}) is reassigned to $ e'\seq f$.
\begin{example}
	We give a pictorial representation of the effect on the derivatives of ${e} \seq {f}$ is given below. Here, we assume that \(\partial({e})\) can perform a \({a}\)-transition to \({e'}\) with probability \({s}\); we make the same assumption in the informal descriptions of derivatives for the loops, below. 
